...
def _default_descriptors() -> tuple[CommandDescriptor, ...]:
    ...
    return (
        ...
        CommandDescriptor(
            ...
            domain="pipeline",
            description="Show governed pipeline session status.",
            kind=CommandKind.READ,
            reads_state=(state_pipeline,),
            output=_output("Pipeline session status.", fields=("sessions", "current_session")),
            validators=("pipeline_session_state",),
            legacy_command=("python scripts/aictl.py pipeline status",),
            notes=("Read-only facade over pipeline session state.",),
        ),
        CommandDescriptor(
            name="pipeline.validate",
            domain="pipeline",
            description="Validate governed pipeline session state and references.",
            kind=CommandKind.VALIDATION,
            reads_state=(state_pipeline, state_tasks, state_evolution, state_task_reports),
            output=_output("Pipeline validation result.", fields=("errors", "warnings")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            legacy_command=("python scripts/aictl.py pipeline validate",),
        ),
        CommandDescriptor(
            name="pipeline.render",
            domain="pipeline",
            description="Render read-only Pipeline Status generated output.",
            kind=CommandKind.RENDER,
            reads_state=(state_pipeline,),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Pipeline status render result.", fields=("path",)),
            validators=("pipeline_session_state", "generated_output"),
            lock_scope="pipeline",
            legacy_command=("python scripts/aictl.py pipeline render",),
            notes=("Generated Pipeline Status is derived output, not source of truth.",),
        ),
        CommandDescriptor(
            name="pipeline.check_generated",
            domain="pipeline",
            description="Check generated Pipeline Status freshness.",
            kind=CommandKind.VALIDATION,
            reads_state=(state_pipeline,),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Pipeline generated-output check result.", fields=("errors",)),
            validators=("pipeline_session_state", "generated_output"),
            legacy_command=("python scripts/aictl.py pipeline check-generated",),
        ),
        CommandDescriptor(
            name="pipeline.policy.list",
            domain="pipeline",
            description="List built-in and custom pipeline policy presets.",
            kind=CommandKind.READ,
            reads_state=(state_pipeline_policy,),
            output=_output("Pipeline policy preset list.", fields=("builtins", "custom")),
            validators=("pipeline_policy_store",),
            legacy_command=("python scripts/aictl.py pipeline policy list",),
            notes=("Read-only facade over built-in policy metadata and custom policy preset state.",),
        ),
        CommandDescriptor(
            name="pipeline.policy.show",
            domain="pipeline",
            description="Show one built-in or custom pipeline policy preset.",
            kind=CommandKind.READ,
            arguments=(_arg("name", "Policy preset name.", required=True),),
            reads_state=(state_pipeline_policy,),
            output=_output("Pipeline policy preset.", fields=("name", "policy")),
            validators=("pipeline_policy_store", "pipeline_policy_validation"),
            legacy_command=("python scripts/aictl.py pipeline policy show <NAME>",),
        ),
        CommandDescriptor(
            name="pipeline.policy.save",
            domain="pipeline",
            description="Save one governed custom pipeline policy preset.",
            kind=CommandKind.WRITE,
            arguments=(
                _arg("name", "Custom policy preset name.", required=True),
                _arg("from_preset", "Built-in preset to copy into the custom preset."),
                _arg("file", "JSON policy payload file."),
                _arg("policy_json", "Inline JSON policy payload."),
            ),
            reads_state=(state_pipeline_policy,),
            writes_state=(state_pipeline_policy,),
            event_logs=("AI_PROJECT/events/pipeline-policy-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_POLICIES.md",),
            output=_output("Saved custom pipeline policy preset.", fields=("name", "action", "policy")),
            validators=("pipeline_policy_store", "pipeline_policy_validation", "generated_output"),
            lock_scope="pipeline_policy",
            owner_approval=(
                "Custom presets are validated before persistence and cannot overwrite built-in presets."
            ),
            legacy_command=("python scripts/aictl.py pipeline policy save --name <NAME>",),
            notes=(
                "Writes only the custom policy preset store through the governed service.",
                "Rejects policies that weaken Token Budget, review, Human Owner, Evolution Change, push, or merge gates.",
            ),
        ),
        CommandDescriptor(
            name="pipeline.policy.delete",
            domain="pipeline",
            description="Delete one governed custom pipeline policy preset.",
            kind=CommandKind.WRITE,
            arguments=(_arg("name", "Custom policy preset name.", required=True),),
            reads_state=(state_pipeline_policy,),
            writes_state=(state_pipeline_policy,),
            event_logs=("AI_PROJECT/events/pipeline-policy-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_POLICIES.md",),
            output=_output("Deleted custom pipeline policy preset.", fields=("name", "action")),
            validators=("pipeline_policy_store", "generated_output"),
            lock_scope="pipeline_policy",
            owner_approval="Built-in presets are immutable and cannot be deleted.",
            legacy_command=("python scripts/aictl.py pipeline policy delete <NAME>",),
        ),
        CommandDescriptor(
            name="pipeline.policy.validate",
            domain="pipeline",
            description="Validate custom pipeline policy preset state and audit events.",
            kind=CommandKind.VALIDATION,
            reads_state=(state_pipeline_policy,),
            event_logs=("AI_PROJECT/events/pipeline-policy-events.jsonl",),
            output=_output("Pipeline policy preset validation result.", fields=("errors", "warnings")),
            validators=("pipeline_policy_store", "pipeline_policy_events"),
            legacy_command=("python scripts/aictl.py pipeline policy validate",),
        ),
        CommandDescriptor(
            name="pipeline.policy.render",
            domain="pipeline",
            description="Render read-only Pipeline Policies generated output.",
            kind=CommandKind.RENDER,
            reads_state=(state_pipeline_policy,),
            generated_files=("AI_PROJECT/generated/PIPELINE_POLICIES.md",),
            output=_output("Pipeline policies render result.", fields=("path",)),
            validators=("pipeline_policy_store", "generated_output"),
            lock_scope="pipeline_policy",
            legacy_command=("python scripts/aictl.py pipeline policy render",),
            notes=("Generated Pipeline Policies is derived output, not source of truth.",),
        ),
        CommandDescriptor(
            name="pipeline.policy.check_generated",
            domain="pipeline",
            description="Check generated Pipeline Policies freshness.",
            kind=CommandKind.VALIDATION,
            reads_state=(state_pipeline_policy,),
            generated_files=("AI_PROJECT/generated/PIPELINE_POLICIES.md",),
            output=_output("Pipeline policies generated-output check result.", fields=("errors",)),
            validators=("pipeline_policy_store", "generated_output"),
            legacy_command=("python scripts/aictl.py pipeline policy check-generated",),
        ),
        CommandDescriptor(
            name="pipeline.session.create",
            domain="pipeline",
            description="Create one governed supervised pipeline session.",
            kind=CommandKind.WRITE,
            arguments=(
                ...
                _arg("max_tasks", "Maximum selected tasks.", value_type="integer"),
                _arg("current_task_id", "Current task ID recorded on the session."),
                _arg("change", "Linked Evolution Change ID.", repeatable=True),
                _arg("report", "Linked execution report ID.", repeatable=True),
            ),
            reads_state=(
                state_pipeline,
            ...
            event_logs=("AI_PROJECT/events/pipeline-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Created pipeline session.", fields=("session_id", "session")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            lock_scope="pipeline",
            owner_approval="Session creation records owner-selected queue and policy; it does not run Codex.",
            legacy_command=("python scripts/aictl.py pipeline session create",),
            notes=(
        ...
        CommandDescriptor(
            ...
            domain="pipeline",
            description="Run one guarded step for a supervised pipeline session.",
            kind=CommandKind.WRITE,
            arguments=(
            ...
            output=_output(
                "Run-next result.",
                fields=("session_id", "stop_code", "selected_task", "queue_counts"),
            ),
            validators=(
            ...
            lock_scope="pipeline",
            owner_approval=(
                "Run-next advances one supervised step and stops before missing gates, "
                "unsafe policies, or unavailable adapters."
            ),
            legacy_command=("python scripts/aictl.py pipeline run-next <SESSION>",),
            notes=(
        ...
        CommandDescriptor(
            name="pipeline.step.start",
            domain="pipeline",
            description="Record the start of one pipeline session step.",
            kind=CommandKind.WRITE,
            arguments=(
                _arg("session_id", "Pipeline session ID.", required=True),
                _arg("step", "Step name.", required=True),
                _arg("task", "Optional task ID associated with the step."),
            ),
            reads_state=(state_pipeline, state_tasks, state_evolution, state_task_reports),
            writes_state=(state_pipeline,),
            event_logs=("AI_PROJECT/events/pipeline-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Started pipeline step.", fields=("session_id", "step")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            lock_scope="pipeline",
            legacy_command=("python scripts/aictl.py pipeline session start-step <SESSION>",),
        ),
        CommandDescriptor(
            name="pipeline.step.result",
            domain="pipeline",
            description="Record the result of one pipeline session step or gate.",
            kind=CommandKind.WRITE,
            arguments=(
                _arg("session_id", "Pipeline session ID.", required=True),
                _arg("step", "Step name.", required=True),
                _arg("result", "Step result.", required=True),
                _arg("gate_name", "Optional gate name."),
                _arg("gate_status", "Optional gate status."),
                _arg("stop_reason", "Optional stop reason."),
            ),
            reads_state=(state_pipeline, state_tasks, state_evolution, state_task_reports),
            writes_state=(state_pipeline,),
            event_logs=("AI_PROJECT/events/pipeline-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Recorded pipeline step result.", fields=("session_id", "step", "gate_outcome")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            lock_scope="pipeline",
            legacy_command=("python scripts/aictl.py pipeline session step-result <SESSION>",),
        ),
        CommandDescriptor(
            name="pipeline.session.stop",
            domain="pipeline",
            description="Stop one active pipeline session with a reason.",
            kind=CommandKind.WRITE,
            arguments=(
                _arg("session_id", "Pipeline session ID.", required=True),
                _arg("reason", "Stop reason.", required=True),
                _arg("status", "Stop status.", default="stopped"),
            ),
            reads_state=(state_pipeline, state_tasks, state_evolution, state_task_reports),
            writes_state=(state_pipeline,),
            event_logs=("AI_PROJECT/events/pipeline-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Stopped pipeline session.", fields=("session_id", "status", "stop_reason")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            lock_scope="pipeline",
            legacy_command=("python scripts/aictl.py pipeline session stop <SESSION>",),
        ),
        CommandDescriptor(
            name="pipeline.session.complete",
            domain="pipeline",
            description="Mark one active pipeline session completed.",
            kind=CommandKind.WRITE,
            arguments=(_arg("session_id", "Pipeline session ID.", required=True),),
            reads_state=(state_pipeline, state_tasks, state_evolution, state_task_reports),
            writes_state=(state_pipeline,),
            event_logs=("AI_PROJECT/events/pipeline-events.jsonl",),
            generated_files=("AI_PROJECT/generated/PIPELINE_STATUS.md",),
            output=_output("Completed pipeline session.", fields=("session_id", "status")),
            validators=("pipeline_session_state", "pipeline_session_references"),
            lock_scope="pipeline",
            legacy_command=("python scripts/aictl.py pipeline session complete <SESSION>",),
        ),
        CommandDescriptor(
            name="docs.render",
            domain="docs",
