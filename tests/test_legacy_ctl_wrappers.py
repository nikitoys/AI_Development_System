...
class LegacyCtlWrapperTests(unittest.TestCase):
    ...
    def test_legacy_commands_keep_outputs_and_audit_events(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            prompt_text = (root / "AI_PROJECT/generated/CODEX_PROMPT.md").read_text(encoding="utf-8")
            self.assertIn("Profile: execute", prompt_text)
            self.assertIn("No Context Pack attached.", prompt_text)
            self.assertNotIn("Retrieved Context Pack Content", prompt_text)
            self.run_ctl("codexctl.py", root, "build", "--task", "TASK-001", "--with-context")
            prompt_text = (root / "AI_PROJECT/generated/CODEX_PROMPT.md").read_text(encoding="utf-8")
            self.assertIn("## Context", prompt_text)
            self.assertIn("Context Pack: AI_PROJECT/generated/CONTEXT_PACK.md", prompt_text)
            self.assertIn("Hash:", prompt_text)
            self.assertIn("Revisions: docs", prompt_text)
            self.assertIn("Refs:", prompt_text)
            self.assertNotIn("Retrieved Context Pack Content", prompt_text)
            self.assertNotIn("Compatibility fixture.", prompt_text)
